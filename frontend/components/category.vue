<template lang="">
    <div>
        <div class="pa-2  text-left primary">List:</div>
          <v-list-item v-for="item in catitems" :key="item.id" class="pl-3 mb-1 catborder">
          ...
          <v-icon color="green darken-2" @click="edit_items(item.id)">mdi-circle-edit-outline</v-icon
            >
            <v-icon color="red darken-2" @click="delete_items(item.id)"
...
export default {
    ...
    edit_items(e) {
      ...
      this.$axios.$get(this.url+'/'+e+'/edit')
      .then(res => {
        this.$emit('cat_edited',res);
        this.$emit('subcat',false);  
      })
      .catch(err => {
        console.error(err); 
      })
    ...
    save_items(e){
      ...
      this.$axios.$put(this.url+'/'+e.id,e)
      .then(res => {
        this.fetch();
        this.dialog=false;
        this.data.msg=res;
